...
def fetch_and_load_year(year: int) -> None:
    import requests
    import pandas as pd
    ...
    logger.info(f"[{year}] Téléchargement des données depuis l'API ODRÉ...")
    ...
    success, nchunks, nrows, _ = write_pandas(
        conn=conn,
        df=df,
        table_name="RAW_NATIONAL_HISTORIQUE",
...
def verify_load() -> None:
    ...
    cursor.execute(
        ...
        "FROM RAW_NATIONAL_HISTORIQUE"
    )
    row = cursor.fetchone()
    conn.close()

    logger.info("✅ CHECK FINALE :")
...
# Définition du DAG
with DAG(
    dag_id="backfill_historique_national",
    description="Charge tout l'historique national éco2mix (2012→2025) dans Snowflake Bronze",
    start_date=datetime(2024, 1, 1),
    schedule=None,
    catchup=False,
    tags=["backfill", "bronze", "national"],
) as dag:

    tasks = []
    for year in range(2012, 2026):
        task = PythonOperator(
            task_id=f"load_year_{year}",
            python_callable=fetch_and_load_year,
            op_kwargs={"year": year},
        )
        tasks.append(task)
